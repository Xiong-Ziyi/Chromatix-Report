...
from chromatix.elements import AmplitudeMask

def chrom_forward(dmd, z):
    field = make_plane_field((H, W), dx, wavelength)
    
    # Construct and apply AmplitudeMask properly
    mask = AmplitudeMask(amplitude=dmd.squeeze((0, 3, 4)), is_binary=True)
    variables = mask.init(jax.random.PRNGKey(0), field)
    field = mask.apply(variables, field)
