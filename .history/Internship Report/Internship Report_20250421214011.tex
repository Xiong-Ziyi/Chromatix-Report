...
\paragraph*{3. \textbf{lenses.py}}
\begin{itemize}
  \item \texttt{ThinLens}: It models the behavior of a thin lens in an optical system by taking three attributes to apply the effect of a thin lens to an incoming field and returns the modified field.
  \item \texttt{FFLens}: This function simulates a situation where a field propagates a distance f to a thin lens, then it passes through the thin lens and then propagates another distance f to the focal plane of the lens.
  \item \texttt{df\_lens}: This function simulates a more general case of 2f setup, where the first f becomes d meaning that the initial field starts from any distance before the thin lens.
\end{itemize}
