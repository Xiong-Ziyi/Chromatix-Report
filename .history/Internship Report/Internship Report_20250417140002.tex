...
\section*{Chromatix Functional API – Module \& Function Overview}

\subsection*{Legend}
\begin{itemize}
  \item \textbf{Name} – public call‐able symbol.
  \item (args) – most important positional arguments.
  \item Short description of what the routine does.
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{amplitude\_masks}}\label{mod:amplitude}
Low‑level helpers for applying spatially varying transmission masks to a complex field.&#8203;:contentReference[oaicite:0]{index=0}&#8203;:contentReference[oaicite:1]{index=1}
\begin{itemize}
  \item \textbf{amplitude\_change}(field, amplitude) – broadcast and multiply a \(H\times W\) mask onto an arbitrary‑rank \texttt{Field}.
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{convenience}}
Utility wrappers that keep frequently used Fourier‑propagation idioms in one line.&#8203;:contentReference[oaicite:2]{index=2}&#8203;:contentReference[oaicite:3]{index=3}
\begin{itemize}
  \item \textbf{optical\_fft}(field, z, n) – Fresnel‐aware forward or inverse 2‑D FFT with correct pixel‑size update.
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{lenses}}
Functions that append ideal thin‑lens phase factors or model free‑space sections surrounding a lens.&#8203;:contentReference[oaicite:4]{index=4}&#8203;:contentReference[oaicite:5]{index=5}
\begin{itemize}
  \item \textbf{thin\_lens}(field, f, n, NA) – multiplies quadratic phase; optional circular pupil.
  \item \textbf{ff\_lens}(field, f, n, NA, inverse) – ``far‑field’’ ( $f\,+\,f$ ) propagation via one FFT.
  \item \textbf{df\_lens}(field, d, f, n, NA, inverse) – generalized \(d\!-\!f\) imager with extra propagation phase.
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{phase\_masks}}
Create, wrap and wavelength‑scale phase distributions.&#8203;:contentReference[oaicite:6]{index=6}&#8203;:contentReference[oaicite:7]{index=7}
\begin{itemize}
  \item \textbf{spectrally\_modulate\_phase}(phase, field)
  \item \textbf{phase\_change}(field, phase, spectrally\_modulate)
  \item \textbf{wrap\_phase}(phase, limits) – custom‑JVP phase wrapping.
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{polarizers}}
Complete Jones calculus toolkit for scalar as well as vectorial fields.&#8203;:contentReference[oaicite:8]{index=8}&#8203;:contentReference[oaicite:9]{index=9}
\begin{itemize}
  \item \emph{Generators of Jones vectors}\\
        \textbf{jones\_vector}, \textbf{linear}, \textbf{left\_circular}, \textbf{right\_circular}
  \item \emph{Primitive matrix applicator}\\
        \textbf{polarizer}(field, J00, J01, J10, J11)
  \item \emph{Ready‑made elements}\\
        \textbf{linear\_polarizer}, \textbf{left\_circular\_polarizer}, \textbf{right\_circular\_polarizer},\\
        \textbf{phase\_retarder}, \textbf{wave\_plate}, \textbf{halfwave\_plate}, \textbf{quarterwave\_plate}, \textbf{universal\_compens}
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{propagation}}
A collection of Fourier‑, angular‑spectrum‑ and single‑FFT propagators plus padding helpers.&#8203;:contentReference[oaicite:10]{index=10}&#8203;:contentReference[oaicite:11]{index=11}
\begin{itemize}
  \item \textbf{kernel\_propagate}(\(field,kernel\))
  \item \textbf{compute\_transfer\_propagator}, \textbf{transfer\_propagate}
  \item \textbf{compute\_exact\_propagator}, \textbf{exact\_propagate}
  \item \textbf{compute\_asm\_propagator}, \textbf{asm\_propagate}
  \item \textbf{transform\_propagate}, \textbf{transform\_propagate\_sas}
  \item \textbf{compute\_sas\_precompensation}
  \item Padding utilities: \textbf{compute\_padding\_transform}, \textbf{compute\_padding\_transfer}, \textbf{compute\_padding\_exact}
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{pupils}}
Ideal hard‑edged spatial filters.&#8203;:contentReference[oaicite:12]{index=12}&#8203;:contentReference[oaicite:13]{index=13}
\begin{itemize}
  \item \textbf{circular\_pupil}(field, diameter)
  \item \textbf{square\_pupil}(field, side)
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{samples}}
Forward models for absorbing, phase and fluorescent specimens (thin or thick).&#8203;:contentReference[oaicite:14]{index=14}&#8203;:contentReference[oaicite:15]{index=15}
\begin{itemize}
  \item \textbf{jones\_sample}, \textbf{thin\_sample}
  \item \textbf{multislice\_thick\_sample}, \textbf{fluorescence\_multislice\_thick\_sample}
  \item \textbf{thick\_sample\_vector}
  \item Numerical helpers: \textbf{PTFT}, \textbf{bmatvec}
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{sensors}}
Image‑formation and shot‑noise simulation.&#8203;:contentReference[oaicite:16]{index=16}&#8203;:contentReference[oaicite:17]{index=17}
\begin{itemize}
  \item \textbf{basic\_sensor}(sensor\_input, \dots) – converts a \texttt{Field} to intensity, optional Poisson/approx.\ noise, resampling and reduction.
\end{itemize}

%--------------------------------------------------------------------
\section{Module \texttt{sources}}
Analytical illumination fields with optional pupil or angular constraints.&#8203;:contentReference[oaicite:18]{index=18}&#8203;:contentReference[oaicite:19]{index=19}
\begin{itemize}
  \item \textbf{point\_source}(shape, dx)
  \item \textbf{objective\_point\_source}(NA, n, \dots)
  \item \textbf{plane\_wave}(shape, dx, direction)
\end{itemize}
